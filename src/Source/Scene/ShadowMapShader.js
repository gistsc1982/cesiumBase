...
ShadowMapShader.createShadowReceiveFragmentShader = function (
  ...
  /**修改by lxf 2021-8-24 */
  /**添加视域模式 */
  if(shadowMap.viewshedMode){
    //如果是平行光源模式，则需要判断是否在视域半径内
    if(isSpotLight){
      fsSource +=
        "    vec3 directionEC_view = positionEC.xyz - shadowMap_lightPositionEC.xyz; \n" +
        "    float distance_view = length(directionEC_view); \n" +
        "    float radius_view = shadowMap_lightPositionEC.w; \n" +
        "    // Stop early if the fragment is beyond the point light radius \n" +
        "    if (distance_view > radius_view) \n" +
        "    { \n" +
        "        return; \n" +
        "    } \n"
    }
    fsSource +=
      "    if(visibility < 1.0)\n"+
      "    {\n"+
      "       gl_FragColor = mix(gl_FragColor,"+shadowMap.invisibleColor+","+shadowMap.alpha+");\n"+
      "    }else{\n"+
      "       gl_FragColor = mix(gl_FragColor,"+shadowMap.visibleColor+","+shadowMap.alpha+");\n"+
      "    }\n"+
      "}\n";
  }else{
    fsSource +=
