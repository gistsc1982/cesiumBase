...
function Timeline(container, clock) {
  ...
  this._topDiv.innerHTML =
    ...
    '</canvas></div><div class="cesium-timeline-needle">'+
    '<span class="cesium-timeline-needle-title"></span>'+
    '</div><span class="cesium-timeline-ruler"></span>';
  ...
  //修改by lxf 2021-9-2 添加时间显示
  this._needleEleTitle = this._topDiv.childNodes[2].childNodes[0];
  //修改by lxf 2021-9-3 添加时区
  this.timeZone = 0; 
...
Timeline.prototype.makeLabel = function (time) {
  ...
  */
  return toDateString(time,this.timeZone);
};
...
Timeline.prototype._makeTics = function () {
  ...
  //修改by lxf 2021-9-2
  this._needleEleTitle.innerHTML = toDateString(this._scrubJulian,this.timeZone);
...
//修改by lxf 2021-9-2
//添加时区转换
function toDateString(time,timezone) {
  var jsDate = JulianDate.toDate(time);
  var offset_GMT = jsDate.getTimezoneOffset();
  var targetDate = new Date(jsDate.getTime() + offset_GMT * 60 * 1000 + timezone * 60 * 60 * 1000);
  //console.log(targetDate);
  return targetDate.getFullYear() +
      '-'+ 
      twoDigits(targetDate.getMonth() + 1) + 
      '-' + 
      twoDigits(targetDate.getDate())  +
      ' '+ 
      twoDigits(targetDate.getHours()) +
      ':'+ 
      twoDigits(targetDate.getMinutes()) + 
      ':' + 
      twoDigits(targetDate.getSeconds());
}
